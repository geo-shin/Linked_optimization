...
class HgsPSO:  # {{{
    def __init__(self, sources_limits, time_list, obs,
            seed:int       =45,
...
nsources   : {self.nsource}
    ...
    def updateParticle(self, part, best, phi1, phi2, iw:int=None, verbose:bool=0): # {{{
        ...
        '''
        pmin, pmax, smin, smax = self.sources_limits[0]
...
class HgsAPSO(HgsPSO):
    def solve(self,Con_min:int=1.0e-5,criteria:int=50,verbose:bool=None, debug:bool=0, phi1:int=2, phi2:int=2): # {{{
        '''
        Explain
         solve PSO
        '''
